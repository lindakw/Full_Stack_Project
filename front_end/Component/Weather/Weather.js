...
console.log("this is info.list==>", info.list);
  ...
  return (
    <View style={styles.container}>
      ...
      />
        ...
        />
        <TouchableOpacity onPress={() => {searchWeather(), setIsSearching(!isSearching), setPlace("")}}>
        <AntDesign
                style={styles.icon}
                name="search1"
                size={28}
       ...
       {showWeatherInfo ? (
        ...
        <View style={styles.displayInfo}> 
          ...
          <View style={styles.highLow}>
            ...
            <Text style={styles.lowText}>L: {Math.round(info.list[0].main?.temp_min)}°F</Text>
          </View>

          <FlatList 
            data = {forecast.splice(0, 7)}
            renderItem ={ ({item,index}) => (
              <View key= {index} style={styles.forcastView}>
                <Text style={styles.infoStyle}>{dayname(item.dt)}</Text>
                <Text style={styles.infoStyle}>{Math.round(item.main.temp)} °F</Text>
              </View>
            )}
